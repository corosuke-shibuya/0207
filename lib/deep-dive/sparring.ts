...
function fallback(input: {
  ...
  const optionA =
    input.mode === "PRE_REFLECT"
      ? "相手が引っかかったのは、私の論点・順序・温度感のどこだったのか。"
      : axes.verbosity === "short"
        ? `いまの発言を踏まえ、事実を3点だけ${phrasing}共有します。`
        : `いまの発言を踏まえ、背景・事実・現状リスクを分けて${phrasing}共有します。`;
  const optionB =
    input.mode === "PRE_REFLECT"
      ? "私が正しい説明をしていても、相手にはどんな印象で伝わっていたのか。"
      : axes.decisionSpeed === "fast"
        ? "維持策と影響最小化策を並べるので、優先順位を今ここで決めてください。"
        : "維持策と影響最小化策を並べるので、判断軸を確認した上で優先順位を決めたいです。";
  const optionC =
    input.mode === "PRE_REFLECT"
      ? "次回同じ相手と話すなら、最初の30秒で何を変えるとズレが減りそうか。"
      : axes.emphasis === "emotional"
        ? "部長視点で、チームの安心感を保つために不足している配慮は何でしょうか。"
        : "部長視点で、意思決定に足りない情報は何でしょうか。";
  const coachingVariants = input.mode === "PRE_REFLECT"
    ? [
        "振り返りでは、まず『何が起きたか』と『自分は何を良かれと思ってやったか』を切り分けてください。今回のズレは、内容の正しさよりも、相手が受け取った順序や温度感にありそうです。次は改善策を急ぐより、相手が止まった瞬間を1つ特定して、その場で自分が何を優先していたかを言語化すると学びが深まります。",
        "今回見るべきなのは、会話が悪化した原因を一つに決め打ちしないことです。あなたの説明・相手の状態・場の文脈のどれが強く効いたかを分けて振り返ると、次回も再現できる気づきになります。いまは『何を直すか』より先に、『私は相手に何を伝えたつもりで、相手には何が伝わったのか』の差分を掘るのが有効です。",
      ]
    : hasQuestion
      ? [
  ...
  const modeRecommendations = {
    PRE_REFLECT: [
      "起きた出来事を『事実』『自分の解釈』『相手の反応』に分けて振り返る。",
      "自分が良かれと思って選んだ伝え方が、相手にどう受け取られたかの差分を言語化する。",
      "次回の改善策を急がず、まず『どの瞬間からズレたか』を1つ特定する。",
    ],
    PRE_STRATEGY: [
  ...
  const followUp = isVague
    ? "一般論を避けるために1点だけ教えてください。今回いちばん引っかかっている場面はどこですか？"
    : input.mode === "FACILITATION"
      ? "次回会議で、あなたが最初の1分で置ける論点整理は何にしますか？"
      : input.mode === "PRE_REFLECT"
        ? "振り返るなら、相手の反応が変わった瞬間はどこだったと感じますか？"
